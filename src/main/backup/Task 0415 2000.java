...
public class Task {
    ...
    public static void add_list() {
        ...
        if (command.toLowerCase().equals("bye")) {
            ...
            delimiter = command.indexOf("/by");
            task = command.substring(0, delimiter);
            date = command.substring(delimiter+3);
            list[counter] = new Deadline(task, date);
        }else if(command.contains("/at")){
            delimiter = command.indexOf("/at");
            task = command.substring(0, delimiter);
            date = command.substring(delimiter+3);
            list[counter] = new Event(task, date);
        }else {
            System.out.println("added: " + command + "\n");
            list[counter] = new ToDo(command);
            counter++;
            add_list();
        }
